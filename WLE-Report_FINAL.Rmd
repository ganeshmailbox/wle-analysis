...
1. Remove the Near Zero variance columns (removes 29 columns)
2. Remove the columns which has 95% of values as NA (removes 77 additional columns)
3. Remove the columns that doesn't make sense in the weight lifting domain. In this case, since we do want to be training the data based on the (Removes additional 5, in total 111 columns is of interest):
...
```{r}
inTrain = createDataPartition(pmlTrain.filtered.2$classe, p=0.7, list=FALSE)
...
gbmfitControl <- trainControl(## 10-fold CV
                           ...
                           ## repeated 3 times
                           repeats = 3)
modFitboost = train(trainingPmlTrain$classe ~., method="gbm", trControl = gbmfitControl, data=trainingPmlTrain, verbose = FALSE)
...
# Variable importance & Out of Sample Error (Validation)
...
**Out-of-Sample Accuracy: `r mat$overall[1]`**. Note that out-of-sample accuracy is better than in-sample accuracy from the model in this case. If accuracy is high, note that sample error is low.
